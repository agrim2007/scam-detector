...
import { ScanLine, Zap, History, Image as ImageIcon, ShoppingCart, Loader2, X, ExternalLink, Tag, Search, TrendingUp } from 'lucide-react';
...
interface ProductResult {
  ...
  score?: number;
  sources?: Array<{ web?: { uri: string; title: string; price?: string } }>;
}

const CameraView: React.FC<{ onCapture: (imageSrc: string) => void }> = ({ onCapture }) => {
    ...
    <div className="relative w-full h-full bg-black text-white overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-20">
        ...
        <div className="flex gap-4">
          <button onClick={() => setFlashOn(!flashOn)} className={`w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md transition-colors ${flashOn ? 'bg-yellow-500/80 text-black' : 'bg-gray-900/60 text-white'}`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 pb-10 pt-20 px-6 flex justify-center items-end bg-gradient-to-t from-black/80 to-transparent z-20">
        <div className="flex items-center gap-8 bg-gray-800/40 backdrop-blur-xl px-8 py-3 rounded-full border border-white/10 shadow-lg">
            ...
            <button onClick={capture} disabled={!isCameraReady} className={`relative group pointer-events-auto transition-all duration-300 ${!isCameraReady ? 'opacity-50 grayscale' : ''}`}>
...
const ResultCard: React.FC<{ result: ProductResult | null; loading: boolean; capturedImage: string; onReset: () => void }> = ({ result, loading, onReset, capturedImage }) => {
  if (loading) return (
  ...
  return (
    <div className="w-full sm:max-w-md bg-gray-900/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl overflow-hidden border border-white/10 shadow-2xl animate-fade-in-up transition-all duration-300 flex flex-col max-h-[90vh] mx-auto">
      <div className="relative h-40 sm:h-48 w-full bg-black/50 flex-shrink-0">
        <img src={capturedImage || result.imageUrl} alt="Product" className="w-full h-full object-cover opacity-80" />
        <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/40 to-transparent" />
        <button onClick={onReset} className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full text-white transition-colors border border-white/10 z-10">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 scrollbar-hide">
        <h2 className="text-lg sm:text-2xl font-bold text-white leading-tight break-words">{result.name}</h2>

        {result.description && (
          <p className="text-gray-400 text-xs sm:text-sm leading-relaxed border-l-2 border-cyan-500/50 pl-2 sm:pl-3">{result.description}</p>
        )}
        ...
        {result.sourceName && (
          <div className="bg-white/5 rounded-lg p-3 border border-white/10">
            ...
            <a href={result.shopUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between p-2 sm:p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 transition-colors group">
        ...
        {result.sources && result.sources.length > 0 && (
          <div className="space-y-2">
            ...
            {result.sources.map((source, idx) => (
              <a key={idx} href={source.web?.uri} target="_blank" rel="noreferrer" className="flex items-center justify-between p-2 sm:p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 transition-colors group">
                <div className="flex items-center gap-2 min-w-0">
                  <ShoppingCart className="w-4 h-4 text-gray-400 group-hover:text-cyan-400 transition-colors flex-shrink-0" />
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 p-3 sm:p-6 border-t border-white/5 flex-shrink-0 bg-gray-900/95 backdrop-blur">
        <button onClick={onReset} className="py-2.5 sm:py-3.5 px-3 sm:px-4 rounded-lg sm:rounded-xl bg-gray-800 hover:bg-gray-700 text-white font-medium sm:font-medium text-sm transition-colors flex items-center justify-center gap-1 sm:gap-2 border border-white/5">
          <Search className="w-4 h-4" />
          <span className="hidden sm:inline">Scan Again</span>
          <span className="sm:hidden">Scan</span>
        </button>
        
        <a href={result.shopUrl} target="_blank" rel="noreferrer" className="py-2.5 sm:py-3.5 px-3 sm:px-4 rounded-lg sm:rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-sm sm:text-base text-center transition-colors flex items-center justify-center gap-1 sm:gap-2 shadow-lg shadow-cyan-500/20">
          <span>Buy Now</span>
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden flex flex-col items-center justify-center font-sans">
      {!capturedImage ? (
        ...
        <div className="relative w-full h-full animate-fade-in">
           <div className="absolute inset-0 bg-cover bg-center opacity-50 blur-xl scale-110" style={{ backgroundImage: `url(${capturedImage})` }} />
           <div className="absolute inset-0 z-10 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
             <ResultCard result={result} loading={isAnalyzing} onReset={handleReset} capturedImage={capturedImage} />
           </div>
        </div>
      )}
    </div>
